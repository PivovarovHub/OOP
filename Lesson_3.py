...
class GamePole:
    ...
    def init(self):
        ...
        q = len(self.pole)
        while k < self.mines:
            x, y = random.randint(0, q-1), random.randint(0, q-1)
        ...
        for x in range(q):
            for y in range(q):
                if not self.pole[x][y].mine:
                    ...
                    mines = sum((self.pole[x+i][y+j].mine for i, j in indx if 0 <= x+i < q and 0 <= y+j < q))
                    self.pole[x][y].around_mines = mines
    
    def show(self):            
        for x in range(len(self.pole)):
            for y in range(len(self.pole)):           
                print('#' if self.pole[x][y].fl_open == False else '*' if self.pole[x][y].mine == True else self.pole[x][y].around_mines, end=' ')
            print()          
